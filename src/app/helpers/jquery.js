  ...
  $.ui.draggable.prototype._generatePosition = function (event, constrainPosition) {
    ...
    if (constrainPosition) {
      if (this.containment) {
        ...
        if (event.pageY - this.offset.click.top * globalScale < containment[1]) {
...
export const resizeSetScaleContainment = (item, globalScale) => {
  ...
  if ($(item).hasClass("ui-draggable")) {
    var containment = [
      ...
      ((containmentArea.offset().left + (containmentArea.width() * globalScale)) -
        ($(item).width() * globalScale)),
      ((containmentArea.offset().top + (containmentArea.height() * globalScale)) -
